...
이 저장소는 다음을 함께 제공하는 FastAPI 기반 Webex Device Assistant 앱입니다.
...
현재 기본값은 **mock-first** 입니다.

- `WEBEX_MOCK_MODE=true`
- `DEVICE_MOCK_MODE=true`

즉, 외부 자격 증명 없이도 앱을 먼저 실행하고 흐름을 검증할 수 있습니다.
...
- Assistant App이 항상 대화와 정책 판단의 중심입니다.
- 실행 모드 차이는 **누가 실행을 담당하느냐** 이지 사용자 경험 차이가 아닙니다.
- 실제 장치 경로는 **로컬 RoomOS 전송이 아니라 Webex cloud xAPI** 입니다.

## 3. 모듈 경계

### `assistant_app/`

앱 조립과 오케스트레이션을 담당합니다.
...
Separated Mode 실행 담당입니다.

- 승인 상태와 지원 인텐트를 확인하고 실제 장치 호출로 연결합니다.

### `direct_tool_adapter/`

All LLM Mode 실행 담당입니다.

- 같은 정규화 요청을 직접 도구 호출 형태로 실행합니다.

### `admin_page/`

브라우저 문서/관리 화면을 정적 파일로 서빙하는 얇은 표면입니다.
...
실제 Webex 메시징을 쓰려면 최소한 다음이 필요합니다.
